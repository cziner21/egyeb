$(document).ready(function() {
	...
	$("#machineResourcesDeleteButton").button();

	
	$("#machineResourcesDeleteButton").click(function() {
			...
			$("<div><p><span class=\"ui-icon ui-icon-alert\" style=\"float:left; margin:0 7px 20px 0;\"></span>Biztos, hogy törlöd a kijelölteket?</p></div>").dialog({ title:"Figyelmeztetés!",autoOpen:true,width:"auto",modal:true,resizable:false,
				...
				buttons: {
					"Törlés": function() {
						...
						var data = $('#machineResourcesForm').serialize();

						$.ajax({
							type: 'post', timeout: 10000, url: 'ajax/machineResources.php?cmd=machineResourcesDelete', data: data, dataType: 'json',
	...
	$("#machineResourcesForm a.customerInformation").click(function() {

		if (runAjax) { return false; }
		runAjax = true;

		$.ajax({
			type: 'post', timeout: 10000, url: 'ajax/machineResources.php?cmd=machineResourcesData', data: { mid: $(this).attr("rel") }, dataType: 'json',
			success: function(json){
				if (json.status == 'success') {		

					$("<div></div>").dialog({ title:""+json.title,autoOpen:true,width:600,modal:false,minWidth:600,minHeight:500,
						close: function(event, ui) {
							$(this).remove();
						},
						open: function() {
