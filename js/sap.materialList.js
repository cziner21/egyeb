$(document).ready(function() {
	...
	$("#materialDeleteButton").button();

	
	$("#materialDeleteButton").click(function() {
			...
			$("<div><p><span class=\"ui-icon ui-icon-alert\" style=\"float:left; margin:0 7px 20px 0;\"></span>Biztos, hogy törlöd a kijelölteket?</p></div>").dialog({ title:"Figyelmeztetés!",autoOpen:true,width:"auto",modal:true,resizable:false,
				...
				buttons: {
					"Törlés": function() {
						...
						var data = $('#materialForm').serialize();

						$.ajax({
							type: 'post', timeout: 10000, url: 'ajax/material.php?cmd=materialDelete', data: data, dataType: 'json',
	...
	$("#materialForm a.customerInformation").click(function() {

		if (runAjax) { return false; }
		runAjax = true;

		$.ajax({
			type: 'post', timeout: 10000, url: 'ajax/material.php?cmd=materialData', data: { mid: $(this).attr("rel") }, dataType: 'json',
			success: function(json){
				if (json.status == 'success') {		
