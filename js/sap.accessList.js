$(document).ready(function() {
	...
	$("#accessDeleteButton").button();
	$("#accessDeleteButton").click(function() {
			...
			$("<div><p><span class=\"ui-icon ui-icon-alert\" style=\"float:left; margin:0 7px 20px 0;\"></span>Biztos, hogy törlöd a kijelölteket?</p></div>").dialog({ title:"Figyelmeztetés!",autoOpen:true,width:"auto",modal:true,resizable:false,
				...
				buttons: {
					"Törlés": function() {
						...
						var data = $('#accessForm').serialize();

						$.ajax({
							type: 'post', timeout: 10000, url: 'ajax/access.php?cmd=userDelete', data: data, dataType: 'json',
	...
	$("#accessForm a.userInformation").click(function() {

		if (runAjax) { return false; }
		runAjax = true;

		$.ajax({
			type: 'post', timeout: 10000, url: 'ajax/access.php?cmd=userData', data: { uid: $(this).attr("rel") }, dataType: 'json',
			success: function(json){
				if (json.status == 'success') {		

					$("<div></div>").dialog({ title:""+json.title,autoOpen:true,width:700,height:600,modal:false,reisze:false,
						close: function(event, ui) {
							$(this).remove();
						},
						open: function() {
