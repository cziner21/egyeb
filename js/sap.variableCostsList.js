$(document).ready(function() {
	...
	$("#variableCostsDeleteButton").button();

	
	$("#variableCostsDeleteButton").click(function() {
			...
			$("<div><p><span class=\"ui-icon ui-icon-alert\" style=\"float:left; margin:0 7px 20px 0;\"></span>Biztos, hogy törlöd a kijelölteket?</p></div>").dialog({ title:"Figyelmeztetés!",autoOpen:true,width:"auto",modal:true,resizable:false,
				...
				buttons: {
					"Törlés": function() {
						...
						var data = $('#variableCostsForm').serialize();

						$.ajax({
							type: 'post', timeout: 10000, url: 'ajax/variableCosts.php?cmd=variableCostsDelete', data: data, dataType: 'json',
	...
	$("#variableCostsForm a.variableCostsInformation").click(function() {

		if (runAjax) { return false; }
		runAjax = true;

		$.ajax({
			type: 'post', timeout: 10000, url: 'ajax/variableCosts.php?cmd=variableCostsData', data: { vid: $(this).attr("rel") }, dataType: 'json',
			success: function(json){
				if (json.status == 'success') {		
